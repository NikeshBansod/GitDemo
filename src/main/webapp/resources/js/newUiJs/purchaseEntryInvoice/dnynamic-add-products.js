...
$(document).ready(function(){
	...
	$("#dnyAddNewProduct").click(function(e){
	...
	$("#dny-productRate,#dny-product-buyingRate").on("keyup input",function(){
		...
		if(dny_prod_currencyRegex.test($("#"+inputId).val()) === true){
			$("#"+spanId).hide();
			$("#"+inputId).addClass("input-correct").removeClass("input-error");	
		}
		if(dny_prod_currencyRegex.test($("#"+inputId).val()) !== true){
	...
	$("#dny-product-os-qty").on("keyup input",function(){
    	...
    	if(currencyRegex.test($("#dny-product-os-qty").val()) === true){
			...
			/*if($("#dny-product-os-qty").val() == 0 ){
				$("#opening-stock-quantity-req").text("Quantity should be greater than 0");
				
			}*/
			if($("#dny-product-buyingRate").val().length != 0) {
				/*if(!checkForZeroQuantity("dny-product-os-qty","opening-stock-quantity-req","Quantity should be greater than 0")){*/
	...
	$("#dny-product-os-value").on("keyup input",function(){
    	...
    	if(currencyRegex.test($("#dny-product-os-value").val()) === true){
			...
			/*checkForZeroQuantity("dny-product-os-value","opening-stock-value-req","Stock Value should be greater than 0");*/
		}
...
function checkDoctypeAndTaxRate(){
	...
	}else if(documentType == 'purchaseEntryInvoiceAndBillOfSupply'){
		isErrorInAddingProduct = false;
	}
