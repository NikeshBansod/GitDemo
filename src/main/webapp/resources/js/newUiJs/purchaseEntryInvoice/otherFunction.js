...
$("#customer_name").autocomplete({
    source: function (request, response) {
        $.getJSON("getSupplierDetailsForAutoCompleteList", {
    ...
    select: function (event, ui) {
        ...
        $.ajax({
			...
			success : function(json,fTextStatus,fRequest) {
				...
				setCsrfToken(fRequest.getResponseHeader('_csrf_token'));
				$("#supplier_dealertye").val(json.custType);
				 $("#supplier_gstin").val(json.custGstId);
				 $("#supplier_contactNo").val(contactNo);
				 $("#supplier_name").val(custName);
				 $("#supplier_place").val(json.custAddress1+","+json.custCity);//mandatory
				 $("#supplier_country").val(json.custState);//mandatory
				 $("#supplier_id").val(json.id);//mandatory
				 $("#supplier_pincode").val(json.pinCode);//mandatory
				 $("#supplier_address").val(json.custAddress1);//optional
				 $("#billing_address").val(json.custAddress1);
				 $("#shipping_address").val(json.custAddress1);
				 //$("#customer_custAddress2").val(json.custAddress2);//optional
				 $("#supplier_city").val(json.custCity);//optional
				 $("#supplier_custState").val(json.custState);//optional
				 $("#customer_custType").val(json.custType);
				 var stateRespone = getStateCode(json.custState);
				 var values = stateRespone.split('{--}');
				 var stateCode = values[0];
