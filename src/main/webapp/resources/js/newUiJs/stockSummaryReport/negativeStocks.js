...
$(document).ready(function () {	
	...
	loadGstinField();
	//createDataTable(negetiveStocksTab);
	$('.loader').fadeOut("slow");
	
	negativereportTable = $('#negetiveStocksTab').DataTable({
        rowReorder: {
	...
	$("#tillDate").datepicker({
        ...
        onSelect: function(selected) {
          ...
          if($('#generateReportGrid').is(':visible')){
        	  $('#negetiveStocksTab').DataTable().row().remove();
          }
	  	  $('#generateReportGrid').hide();    
        }
	});
...
function loadReport(gstinNo,storeId,tDate){
	$.ajax({
		...
		success:function(json,fTextStatus,fRequest){				
			...
			console.log(json);
			$('.loader').show();
			negativereportTable.clear().draw(); 
			counter = 0;
				 	...
				 	$.each(json.result,function(i,value){
				 		if( value.currentStock <= 0){
					 		...
					 		if(value.increment === 'null'){
					 			...
					 			incrementQty = value.increment;
					 		}

					 		var decrementQty;
					 		if(value.decrement === 'null'){
					 			decrementQty = '-';
					 		}else{
					 			decrementQty = value.decrement;
					 		}
				 		
				 			MyUniqueID=counter;
				 			negativereportTable.row.add($(
