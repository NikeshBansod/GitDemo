		 ...
		 financialPeriod = $("#financialPeriod").val();
			...
			if((errGSTIN)){
				 ...
				 if(getGstinName == true ){
					 ...
					 }else  if(gstrtype=='GSTR1'||GSTR_type=='GSTR1'){
				 $(".selectOptions").show();
				 $(".selectOptionsgstr2").hide();
				 }
				/* else if(gstrtype=='GSTR3B'){
					...
					bootbox.confirm({
							message :"<form name='saveGstinUId' id='saveGstinUId' method='post'>\
							    ...
							    <input type='text' id='gstnUserId'  name='gstnUserId' />\
							    ...
							    <input type='number' id='grossTurnover'  name='grossTurnover' />\
							    ...
							    <input type='number' id='currentTurnover' name='currentTurnover' />\
		...
		success : function(json,fTextStatus,fRequest) {
			...
			/*if(json.UploadToJiogst==0&&json.SaveToGstn==0&&json.SubmitToGstn==0)
			 ...
			 if(button=="draft")
				{
				 ...
				 {
					 bootbox.alert('Please upload '+json.UploadToJiogst+' invoices to JioGST.');
				 }
				}
			 ...
			 else
			 {
				 if(json.UploadToJiogst==0)
					 {
					 bootbox.alert('Please save '+json.SaveToGstn+' invoices to Gstn. ');
					 }
				 else{
				 bootbox.alert('Please upload '+json.UploadToJiogst+' invoices to JioGST  ');
				 }
				 ...
				 else
				 {
					 if(json.UploadToJiogst==0)
					 {
						 if(json.SaveToGstn==0)
							 {
					 bootbox.alert('Please submit '+json.SubmitToGstn+' invoices to Gstn. ');
							 }
						 else
						 {
							 bootbox.alert('Please save '+json.SaveToGstn+' invoices to Gstn. '); 
