...
$(document).ready(function(){
		...
		//	 var errFlag   = validateOrgName(); 
			...
			/* 
			 ...
			 if (  (errFlag1) || (errFlag8)  || (userMobileNoExist) || (panRegistered) || (errFlag20) || (errFlag10) || (errFlag11) || (errFlag12)){
				// (errFlag) ||  (errFlag3) || 
				 e.preventDefault();
				 ...
				 var result = false;
		   		 if(!result){
		   			...
		   			//verifyUserIdAndSendOtp(contactNo,emailId,pannumber,password);
			...
			function validateEmail(){
				errFlag12 = validateTextField("emailId","emailId-req",blankMsg);
				 if(!errFlag12){
					 errFlag12=validateRegexpressions("emailId","emailId-req", regMsg, emailRegex);
				 }
				 return errFlag12;
			}
 ...
 //   $("#RegisterBtn").attr("disabled", "disabled"); 
    ...
    $("#verifyOtp").click(function(e){
   	 ...
   	 if($("#otp").val()==""){
   		 ...
   		 $("#otp").addClass("input-correct").removeClass("input-error");
   		 $("#otp-req").hide();
   		 verifyOtp(otp,cachekey,contactNo,emailId,pannumber,password);
   	 }
...
function verifyUserIdAndSendOtp(){
	    ...
	    $.ajax({
			url : "registrationunification",
			type : "POST",
			dataType : "json",
			headers: {_csrf_token : $("#_csrf_token").val()},
			data:{"contactNo":contactNo,"emailId":emailId,"pannumber":pannumber,"password":password},
			async : false,
			success : function(jsonVal,fTextStatus,fRequest) {
				if (isValidSession(jsonVal) == 'No') {
					window.location.href = getDefaultSessionExpirePage();
				...
				if(jsonVal.msg){
					if(jsonVal.msg.status_cd){
						if(jsonVal.msg.status_cd == "1"){
							bootbox.alert("OTP has been sent to Mobile No : "+contactNo+". Kindly Verify to complete Registration");
							$("#divOtp").show();
							if(jsonVal.cachekey){
								cachekey = jsonVal.cachekey;
							}
						}
...
function verifyOtp(otp,cachekey,contactNo,emailId,pannumber,password){
	    $.ajax({
			url : "verifyRegistrationUnifiOtp",
			type : "POST",
			dataType : "json",
			data:{"otp":otp, "cachekey":cachekey,"contactNo":contactNo, "emailId":emailId,"pannumber":pannumber, "password":password},
			headers: {_csrf_token : $("#_csrf_token").val()},
			async : false,
			success : function(jsonVal,fTextStatus,fRequest) {
				...
				if(jsonVal.msg){
					if(jsonVal.msg.status_cd){
						if(jsonVal.msg.status_cd == "1"){
								bootbox.alert("User successfully registered" , function() {
									window.location.href = "./loginunifi";
								   });
						}
						else if(jsonVal.msg.status_cd == "0"){
							if(jsonVal.msg.status_msg){
