...
$("#service_add,#service_edit").on( "click", function(e) {
	...
	var locationErrFlag = validateSelectField("fromlocation","location-csv-id");
	var errFlagTransactionDate = validateTextField("transactionDate","transactionDate-csv-id",blankMsg);
	
	
	
	if (productNameFlag || quantityFlag  || stockValueFlag || gstinErrFlag || locationErrFlag  || errFlagTransactionDate||uomFlag){ 
		e.preventDefault();
...
$("#search-product-autocomplete").autocomplete({
    ...
    select: function (event, ui) {
        ...
        dynamicallyAddedProductJson = fetchDynamicallyAddedProduct($("#fromlocationId").val(),value.trim());
...
function resetFormValues(){
 ...
 function getProductList(){
		...
		var gNo = $("#fromgstin option:selected").text().split('[')[0].trim();
        var location = $('select#fromlocation option:selected').val();
		//var location='3';
			 $(document).ready(function(){
				    $.ajax({
	    ...
	    function checkIfGstinAndLocationIsSelected(){
	    	...
	    	//fetch gstin number like 27XXXXX1234A1ZQ[Maharashtra]
	    	 var gstnWithStateInString = $("#fromgstin option:selected").text();
	    	 var gstnStateIdInString = (gstnWithStateInString.split('[')[0]).trim();
	    	 var gstnStateId = $('select#fromgstin option:selected').val();
	    	 if(gstnStateIdInString == undefined || gstnStateIdInString == 'Select' || gstnStateId == ''){ 
	    		   alert("Please Select GSTIN");
	    	 }else{
	    		 
	    		//fetch location
	    		 var locationVal = $('select#fromlocation option:selected').val();
	    ...
	    $('#finalSave').on('click', function(e){
	   	 $('.loader').show();
	   	    	  ...
	   	    	  var totalRecordNo = $toggle.children().length;
	   	   	   	  var productJsonObject;
	   	   	   	  var gstin = $('#fromgstin').val();
	   	   	   	  var locationUniqueSequence = $('#fromlocation').val();
	   	   	   	  var location = $('#fromlocationId').val();
	   		   ...
	   		   for (i = 0; i < totalRecordNo; i++) { 
	   				...
	   				if(!alertFlagQty && !alertFlagQtyNull && !alertFlagValue && !alertFlagValueNull){
	   				 ...
	   				 productJsonObject.unitOfMeasurement = $("#uom"+num2).val();
	   				 productJsonObject.otherUOM = $("#uom"+num2).val();
	   				 productJsonObject.currentStock = checkNConvertToDecimal(oldQty);
	   				 productJsonObject.currentStockValue = parseFloat(oldStockValue).toFixed(2);
	   				 productJsonObject.fromStoreId = location;
	   				 productJsonObject.toStoreId = tolocation;
	   				 productJsonObject.modifiedQty = newQty;
	   				 productJsonObject.modifiedStockValue = parseFloat(newStockValue).toFixed(2);
	   				 productJsonObject.transactionDate = transactionDate;
	   				 productListArray.push(productJsonObject);
