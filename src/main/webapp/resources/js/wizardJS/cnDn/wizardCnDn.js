...
$(document).ready(function () {
	...
	$('.taxRate,.taxableValue,.cess').on('change blur', function(){
		var zid = $(this).attr('id');
		var index = zid.substring(zid.lastIndexOf("-")+1);
   ...
   $('.cess').on("keyup change click", function(){
	   var zid = $(this).attr('id');
	   var index = zid.substring(zid.lastIndexOf("-")+1);
		var indexOfCess=zid.indexOf('cess');
	   if($(this).val() >= 0){
		//   if(zid.startsWith('cess')){
		if(zid.substring(0, indexOfCess) == 'cess'){
...
function dynamicChangesInPreviewLabels(){
	...
	if(cnDnType === 'creditNote'){
		...
		$("#finalSubmitId").prop('value', 'Send Credit Note');
	}else{
		...
		$("#finalSubmitId").prop('value', 'Send Debit Note');
	}
}
			...
			success:function(json,fTextStatus,fRequest){
				...
				if(json.response == 'FAILURE'){
					bootbox.alert("Failed to generate"+cnDnTypeText+".", function() {
							setTimeout(function(){ 
								window.location.href = getWizardHomePage();
								return;
							}, 1000);
					});
				}
			...
			success:function(json,fTextStatus,fRequest){
				...
				if(json.renderData === 'accessDeny'){
					bootbox.alert("Data is manipulated.", function() {
						setTimeout(function(){
							window.location.href = callWizardInvoiceHistoryPage();
						}, 1000);
					});
				}else{
	  ...
	  if($("#footerNote").val()){
		  $("#footerNoteDiv").show();
			$("#footerNoteDiv").append(
					'<div class="add-charges" >'+$("#footerNote").val()+'</div>'
...
function backToInvoicePage(idValue) {
	document.previewInvoice.action = "./getWizardCnDn";		//getInvoiceCNDNDetails
